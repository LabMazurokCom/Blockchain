...
#     'btc_usd': {
#         'orders': {
#             'asks': [[price_with_fee, volume, exchange_name, price_original], ...],
#             'bids': [[], ...]
#         }
#     },
#     'eth_btc': {
...
def get_arb_opp(order_books, current_balance, alpha=0.1):
    ...
    for pair in order_books.keys():
        ...
        while bx < bid_count and ax < ask_count and bids[bx][0] > asks[ax][0]:
            ...
            print(ask_price, ask_vol, ask_exch, bid_price, bid_vol, bid_exch)

            try:
                m = min(ask_vol, bid_vol, ask_bal / ask_price_real, bid_bal)  # current micro-trade volume
                print(ask_exch, bid_exch, m)
                ...
                print("ask_bal", ask_bal)
                print("ask_price_real", ask_price_real)
                print("bid_bal", bid_bal)
                print("prev_quote_amount", prev_quote_amount)
                try:
                    x = ask_bal / ask_price_real
                    print("ask_bal / ask_price_real", x)
                    print("m", m)
                    print("quote_amount", quote_amount)
                    print("quote_amount - prev_quote_amount", quote_amount - prev_quote_amount)
                except ZeroDivisionError:
                    print("ask_bal / ask_price_real 0")
                    base_amount += m
